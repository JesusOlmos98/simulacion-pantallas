...
export default function ObjEditVariablesString({ value, onChange, onEnter, responsive }: Props): JSX.Element {
  return (
    <div className={`flex flex-col items-center ${responsive ? 'gap-4' : 'gap-8'}`}>
      <div
        className={`flex items-center rounded-2xl ${responsive ? 'px-4 py-3' : 'px-8 py-5'}`}
      ...
      >
        <input
          ...
          className={`bg-transparent text-center outline-none ${responsive ? 'text-2xl w-48' : 'text-6xl w-120'}`}
          style={{ color: COLORES.success }}
          autoFocus
        />
      </div>
      <span className={`${responsive ? 'text-sm' : 'text-2xl'} text-white/60`}>
