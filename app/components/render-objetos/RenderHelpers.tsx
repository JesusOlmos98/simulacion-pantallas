...
function navegarSimple(nav: number, indicePantalla: number, onNavegar: OnNavegar): void {
  if (nav <= 0) return;
...
function navegarConEdicion(nav: number, indicePantalla: number, onNavegar: OnNavegar): void {
  if (nav <= 0) return;
  onNavegar({ idPantalla: nav, indicePantalla, esPrincipal: false, idUnicoEdicion: nav });
}

/** Componente ChevronRight configurable */
function ChevronRight({ size = 28 }: { size?: number }): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
