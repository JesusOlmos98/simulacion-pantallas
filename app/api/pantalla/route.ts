// Centralizado en app/api/apiFetch.ts — este route ya no se usa.
export {};
//
...
//   try {
//     commacRes = await fetch(url.toString(), { method: 'POST', signal: AbortSignal.timeout(35_000) });
//   } catch (err) {
...
//
//   if (!commacRes.ok) {
//     return Response.json({ error: `COMMAC respondió con ${commacRes.status}` }, { status: commacRes.status });
//   }
//
